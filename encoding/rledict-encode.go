...
func RLEDictEncode(values []interface{}, parquetType parquet.Type, bitWidth int32) (dictPageData, dataPageData []byte, valueCount, indexCount, indexBitWidth int) {
	...
	indexBitWidth = int(common.BitWidth(uint64(indices[len(indices)-1])))
	...
	return dictPageData, dataPageData, len(definedValues), len(indices), indexBitWidth
}
